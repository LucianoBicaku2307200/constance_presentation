...
> {
    ...
    private selectDefaultShippingOptions: (state: CheckoutSelectors) => void = async ({ data }) => {
        ...
        const freeShipping = availableShippingOptions.find(el=>el.type == 'freeshipping');

        const flatRate = availableShippingOptions.find((el) => el.type == "shipping_flatrate" && !el.description.includes("COD"));

        await selectShippingOption(id, freeShipping?.id ? freeShipping?.id : flatRate?.id ? flatRate?.id : defaultShippingOption.id);
