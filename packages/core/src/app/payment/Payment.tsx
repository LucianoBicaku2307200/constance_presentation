...
> {
  ...
  private setSelectedMethod: (method?: PaymentMethod) => void = (method) => {
    ...
    this.changeShippingMethod(method?.id == 'cod');
    analyticsTracker.selectedPaymentMethod(method?.config.displayName);

    this.setState({ selectedMethod: method });
  };

  private changeShippingMethod: (isCOD: boolean) => void = async (isCOD) => {
    const { consignments, selectShippingOption } = this.props;
    const consignment = consignments && consignments[0];

    if (consignment) {
      const { id, availableShippingOptions, shippingAddress } = consignment;

      const freeShipping = availableShippingOptions?.find((el: any) => el.type == 'freeshipping');
      const freeShippingWithCOD = availableShippingOptions.find(
        (el: any) => el.type == 'shipping_flatrate' && el.cost == 0,
      );
      const flatRateWithCOD = availableShippingOptions.find(
        (el: any) =>
          el.description.includes('COD') && el.type == 'shipping_flatrate' && el.cost !== 0,
      );
      const flatRate = availableShippingOptions.find(
        (el: any) => el.type == 'shipping_flatrate' && !el.description.includes('COD'),
      );

      if (shippingAddress.countryCode == 'IT')
        if (isCOD) {
          freeShipping
            ? freeShippingWithCOD && (await selectShippingOption(id, freeShippingWithCOD.id))
            : flatRateWithCOD && (await selectShippingOption(id, flatRateWithCOD.id));
        } else {
          freeShipping
            ? freeShipping && (await selectShippingOption(id, freeShipping.id))
            : flatRate && (await selectShippingOption(id, flatRate.id));
        }
    }
  };
