...
> = ({ cdnBasePath, onUnhandledError, formik: { values }, instruments, isSelected, language, method }) => {
    ...
    const configData = JSON.parse(window.sessionStorage.getItem("CodConfigData") || "");
    ...
    return (
        <div className="paymentProviderHeader-container">
            ...
            >
                ...
                {titleText && (
                    <div aria-level={6} className={method.id !== "cod" ?"paymentProviderHeader-name": "paymentProviderHeader-name-cod"} data-test="payment-method-name" role="heading">
                        {titleText}
                        {method.id === "cod" && configData && <span className='shippingOption-price'>{configData.fee}</span> }
                    </div>
                )}
